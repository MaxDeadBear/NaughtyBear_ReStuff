...
macro(rexglue_setup_target target_name)
    if(REXGLUE_ENTRYPOINT_GENERATED_SOURCES)
        ...
        rexglue_apply_recomp_settings(${target_name}_recomp
            "${REXGLUE_ENTRYPOINT_INCLUDE_DIR}/restuff_pch.h")
        add_dependencies(${target_name}_recomp restuff_codegen)
        target_link_libraries(${target_name} PRIVATE
            ${target_name}_recomp)
    endif()
    # Also on the host, so a tree that has never run codegen still generates it.
    add_dependencies(${target_name} restuff_codegen)
...
# changes. Build it alone with 'cmake --build . --target restuff_codegen'.
...
# names it.
# LOCAL PATCH (M4.28): the codegen CLI and its emitter env are overridable.
# Closes two traps: the in-build regen silently used the stock found-package
# CLI (replacing fused-FMA generated code with unfused emission), and a
# Linux->Windows cross build imports rex::rexglue as an unrunnable .exe.
set(REXGLUE_CODEGEN_CLI "$<TARGET_FILE:rex::rexglue>" CACHE STRING
    "rexglue CLI used for in-build codegen (must be host-runnable)")
set(REXGLUE_CODEGEN_ENV "" CACHE STRING
    "Codegen-time env assignments (semicolon list, e.g. REX_EMIT_FMA=1)")
add_custom_command(
    OUTPUT "${CMAKE_CURRENT_SOURCE_DIR}/generated/default/codegen.build.stamp"
           ${REXGLUE_ENTRYPOINT_GENERATED_SOURCES}
    COMMAND ${CMAKE_COMMAND} -E env ${REXGLUE_CODEGEN_ENV} ${REXGLUE_CODEGEN_CLI} codegen ${CMAKE_CURRENT_SOURCE_DIR}/restuff_manifest.toml
    DEPFILE "${CMAKE_CURRENT_SOURCE_DIR}/generated/default/codegen.d"
    WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
    COMMENT "Generating recompiled code for restuff"
    VERBATIM
)
add_custom_target(restuff_codegen
